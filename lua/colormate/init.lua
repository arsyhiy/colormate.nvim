...
-- local treesitter = require("colormate.integrations.treesitter")   -- ← добавь
-- local lsp = require("colormate.integrations.lsp")               -- ← добавь
-- local telescope = require("colormate.integrations.telescope")    -- ← добавь
-- local nvimtree = require("colormate.integrations.nvimtree")      -- ← добавь
...
-- 	local diff_add = utils.shade(colorscheme.successText, 0.40, colorscheme.editorBackground)
-- 	local diff_delete = utils.shade(colorscheme.syntaxError, 0.40, colorscheme.editorBackground)
-- 	local diff_change = utils.shade(colorscheme.syntaxFunction, 0.40, colorscheme.editorBackground)
-- 	local diff_text = utils.shade(colorscheme.warningEmphasis, 0.40, colorscheme.editorBackground)
--
-- 	local groups = {
-- 		-- base
...
-- 		ColorColumn = {
-- 			bg = utils.shade(colorscheme.linkText, 0.040, colorscheme.editorBackground),
-- 		},
-- 		Conceal = {},
...
-- 		PmenuSbar = {
-- 			bg = utils.shade(colorscheme.syntaxFunction, 0.30, colorscheme.editorBackground),
-- 		},
...
-- 		},
-- 		Search = { bg = utils.shade(colorscheme.stringText, 0.40, colorscheme.stringText) }, --bg
...
-- 	groups = vim.tbl_extend("force", groups, treesitter.highlights())
-- 	groups = vim.tbl_extend("force", groups, lsp.highlights())
-- 	groups = vim.tbl_extend("force", groups, telescope.highlights())
-- 	groups = vim.tbl_extend("force", groups, nvimtree.highlights())
--
-- 	-- overrides
-- 	groups =
...
-- 		vim.notify("Neovim 0.8+ is required for colormate colorscheme", vim.log.levels.ERROR, { title = "Min Theme" })
-- 		return
-- 	end
--
...
-- lua/colormate/init.lua
local bufferline = require("colormate.integrations.bufferline")
local cmp = require("colormate.integrations.cmp")
local treesitter = require("colormate.integrations.treesitter")
local lsp = require("colormate.integrations.lsp")
local telescope = require("colormate.integrations.telescope")
local nvimtree = require("colormate.integrations.nvimtree")
...
local function set_groups()
	...
	local groups = {
		...
		Normal = { fg = colorscheme.mainText, bg = bg },
		LineNr = { fg = colorscheme.lineNumberText },
		ColorColumn = { bg = utils.shade(colorscheme.linkText, 0.040, colorscheme.editorBackground) },
		Conceal = {},
		Cursor = { fg = colorscheme.editorBackground, bg = colorscheme.mainText },
		lCursor = { link = "Cursor" },
		CursorIM = { link = "Cursor" },
		...
		Folded = { fg = colorscheme.mainText, bg = colorscheme.popupBackground },
		FoldColumn = { link = "SignColumn" },
		IncSearch = { bg = utils.mix(colorscheme.syntaxFunction, colorscheme.editorBackground, math.abs(0.30)), fg = colorscheme.editorBackground },
		Substitute = { link = "IncSearch" },
		CursorLineNr = { fg = colorscheme.commentText },
		MatchParen = { fg = colorscheme.syntaxError, bg = bg },
		ModeMsg = { link = "Normal" },
		...
		Pmenu = { link = "NormalFloat" },
		PmenuSel = { bg = colorscheme.menuOptionBackground },
		PmenuSbar = { bg = utils.shade(colorscheme.syntaxFunction, 0.30, colorscheme.editorBackground) },
		PmenuThumb = { bg = utils.shade(colorscheme.editorBackground, 0.20) },
		Question = { fg = colorscheme.syntaxFunction },
		QuickFixLine = { fg = colorscheme.syntaxFunction },
		SpecialKey = { fg = colorscheme.syntaxOperator },
		...
		TabLineFill = { link = "TabLine" },
		TabLineSel = { bg = colorscheme.editorBackground, fg = colorscheme.emphasisText },
		Search = { bg = utils.shade(colorscheme.stringText, 0.40, colorscheme.stringText) },
		SpellBad = { undercurl = true, sp = colorscheme.syntaxError },
		SpellCap = { undercurl = true, sp = colorscheme.syntaxFunction },
		SpellLocal = { undercurl = true, sp = colorscheme.syntaxKeyword },
		SpellRare = { undercurl = true, sp = colorscheme.warningText },
		...
		-- Tree-Sitter (старые группы, которые уже были у тебя)
		["@text"] = { fg = colorscheme.mainText },
		["@texcolorscheme.literal"] = { link = "Property" },
		["@texcolorscheme.strong"] = { link = "Bold" },
		["@texcolorscheme.italic"] = { link = "Italic" },
		["@texcolorscheme.title"] = { link = "Keyword" },
		["@texcolorscheme.uri"] = { fg = colorscheme.syntaxFunction, sp = colorscheme.syntaxFunction, underline = true },
		["@texcolorscheme.underline"] = { link = "Underlined" },
		["@symbol"] = { fg = colorscheme.syntaxOperator },
		["@texcolorscheme.todo"] = { link = "Todo" },
		["@comment"] = { link = "Comment" },
		...
		["@operator"] = { link = "Operator" },
		["@exception"] = { link = "Exception" },
		["@variable"] = { fg = colorscheme.syntaxFunction, italic = config.italics.variables or false },
		["@variable.builtin"] = { fg = colorscheme.syntaxFunction },
		["@variable.member"] = { fg = colorscheme.mainText },
		["@variable.parameter"] = { fg = colorscheme.mainText, italic = config.italics.variables or false },
		["@type"] = { link = "Type" },
		["@type.definition"] = { fg = colorscheme.mainText },
		["@type.builtin"] = { fg = colorscheme.syntaxFunction },
		["@type.qualifier"] = { fg = colorscheme.syntaxFunction },
...
function theme.colorscheme()
	if vim.version().minor < 8 then
		vim.notify("Neovim 0.8+ is required for colormate colorscheme", vim.log.levels.ERROR, { title = "Colormate" })
		return
	end
